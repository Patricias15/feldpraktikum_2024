...
def main(plotroutine=None, excel_filename=None, var_dict=None, figurename=None, titlestr=None, flag=None, timebegin=None, timeend=None, timemarker=None, hour_interval=3, time_freq='D'):
    ...
    if plotroutine == 'hobo_multi':
        ...
        precip_num = [float(str(ti).split(',')[0].replace(' ', '')) for ti in precip_ticks]
    ...
    # synoptic observations
        ...
        # omniport
        T_omniport = df['T_omniport'].values
        RH_omniport = df['RH_omniport'].values
        ...
        p5 = plot_t_td_rh(ax1, axr1, time, T_omniport, np.zeros(len(time))*np.nan, RH_omniport, label=Lomniport, color=Comniport)
